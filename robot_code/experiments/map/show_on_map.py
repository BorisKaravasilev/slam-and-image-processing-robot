...
def main():
    ...
    #GOTOTARGET(x,y,theta, x-trget, y-target)

    #1 x-distance = x-target-x
    # y-distance = y-target-y
    # x-distance in mm = x-distance*60 
    # y-distance in mm = y-distance*60 
    #
    ...
    def driveToTarget(targetX, targetY):
        

        if grid_x != targetX and grid_y !=targetY:
            robot.drive(100, 100)
        else:
            robot.stop()
    ...
    while True:
        ...
        try:
            ...
            grid_x, grid_y = getGridPosition(getPosition())
            angle = getPosition()[2]
            print(f"grid_x = {grid_x} grid_y = {grid_y} angle = {angle}")

            # print_map(mapbytes)
            show_map_in_window(mapbytes, int(grid_x), int(grid_y), angle)

            # rotateToTarget()
            #driveToTarget(goalX, goalY)
            # robot.stop()
        
            
            #robot.drive(-100, 100)
            #driveToTarget(slam.getpos(), 128, 128)
